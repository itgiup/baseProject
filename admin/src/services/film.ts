...
import { FilmState } from "@pages/Film/constant";
import { AxiosResponse } from "axios";
import { API_BASE_URL } from "@configs/index";
const pathApi = "film";
interface FilmApiState extends ApiState<FilmState> {
  assignProfile: (id: string, data: any) => Promise<AxiosResponse<ApiResponse<any>>>;
  all: () => Promise<AxiosResponse<ApiResponse<FilmState[]>>>;
  isLogin: () => Promise<AxiosResponse<ApiResponse<any>>>;
}
const api: FilmApiState = {
  getAll: (data) => {
