...
import { TIMEZONESType } from "../utils/std";
...
export const importSetting = createAsyncThunk(
    "importSetting",
    async (_settings: any, thunkAPI: { getState: any; }) => {
...
async function _loadSettings(): Promise<SettingsInitialType> {
    let settings: SettingsInitialType | null = await localforage.getItem("settings")
    if (settings)
        return settings
...
export const change = createAsyncThunk(
    "change",
    async (args: any, thunkAPI: { getState: () => any; }): Promise<any> => {
...
export const { reset, toggleDark, } = settingsSlice.actions;
